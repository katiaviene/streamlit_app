...
class Alternatives:
    ...
    def render(self):
        ...
        with c2:
            ...
            st.write("---")
            st.write(
                "Out-of-the-box instant cloud hosting , drag-and-drop UI builder, more features for complex apps")
            st.write(
                "Larger learning curve, not all features are available for free")
            st.write("---")
            st.write("Any Jupyter Notebook can also be a Voila dashboard.")
            st.write(
                "More complex dashboards with multiple interacting parts are harder to build.")
